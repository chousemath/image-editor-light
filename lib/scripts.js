...
var loadImages = function loadImages(images) {
  ...
  var ngrokInput = document.getElementById('ngrok').value;
  ...
  var _loop = function _loop(i) {
    ...
    if (ngrokInput) images[i] = "".concat(ngrokInput, ".ngrok.").concat(images[i].split('.ngrok.')[1]);
    img.src = images[i];
...
var forbidden = ['dic.png', 'blank.png', 'dic2.png', 'search.png', 'search2.png', 'internet.png', 'configure.png', 'close.png', 'forward.png'];

function placeFileContent(file) {
  readFileContent(file).then(function (content) {
    loadImages(content.split('\n').filter(function (im) {
      return im && forbidden.some(function (x) {
        return im.indexOf('chrome-extension') === -1;
      });
    }));
